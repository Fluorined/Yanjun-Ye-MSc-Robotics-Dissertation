# tracking_val.py - PID控制版本可视化脚本

import os
import numpy as np
import matplotlib.pyplot as plt
from rl_pid import RRTTrackingEnv, generate_obstacles, rrt, collision_free
...
def analyze_pid_parameters(model, env, n_samples=20):
    """分析模型学到的PID参数"""
    print("\n=== PID参数分析 ===")
...
def visualize_single_episode(model, vec_env, episode_num, seed):
    ...
    while not done:
        ...
        if isinstance(action, np.ndarray) and action.ndim > 1:
            pid_params = env._map_action_to_pid_params(action[0])
        else:
            pid_params = env._map_action_to_pid_params(action)
        ...
        info_str = (
            ...
            f"PID-X: Kp={pid_params[0]:.2f}, Ki={pid_params[1]:.2f}, Kd={pid_params[2]:.2f}\n"
            f"PID-Y: Kp={pid_params[3]:.2f}, Ki={pid_params[4]:.2f}, Kd={pid_params[5]:.2f}\n"
            f"PID误差: ({env.pid_controller.error_x:.2f}, {env.pid_controller.error_y:.2f})"
        )
    ...
    if plt.fignum_exists(1):
        ...
        final_text = (
            ...
            f"路径点: {env.traj_idx}/{len(env.traj)-1}\n"
            f"种子: {seed}\n"
    ...
    print(f"PID最终状态: 误差=({env.pid_controller.error_x:.2f}, {env.pid_controller.error_y:.2f})")
    print(f"PID积分项: ({env.pid_controller.integral_x:.2f}, {env.pid_controller.integral_y:.2f})")
...
def visualize_trained_model_custom(model_path, num_episodes=3, seed=42, start=(0.5, 0.5), goal=(9.5, 9.5)):
    ...
    if hasattr(vec_env, 'normalize_obs'):
        ...
        # 创建一个包装函数来处理归一化
        def predict_with_normalization(obs, **kwargs):
            obs_normalized = vec_env.normalize_obs(obs)
            return original_predict(obs_normalized, **kwargs)
        
        # 临时替换模型的预测方法
        model.predict = predict_with_normalization
        pid_stats, pid_array = analyze_pid_parameters(model, env, n_samples=20)
        model.predict = original_predict  # 恢复原始方法
    else:
        pid_stats, pid_array = analyze_pid_parameters(model, env, n_samples=20)
...
if __name__ == "__main__":
    # 配置参数 - PID版本
    MODEL_BASE_PATH = "./results/rl_pid_-1to1"  # PID版本模型目录
